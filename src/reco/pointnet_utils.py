...
class PointNetMLPEncoder(nn.Module):
    def __init__(self, global_feat=True, feature_transform=False, channel=3):
        super(PointNetMLPEncoder, self).__init__()
        self.stn = STN3d(channel)
        self.conv1 = torch.nn.Conv1d(channel, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        self.conv3 = torch.nn.Conv1d(128, 1024, 1)
...
class PointNetModMLPEncoder(nn.Module):
    def __init__(self, global_feat=True, feature_transform=False, channel=3):
        super(PointNetModMLPEncoder, self).__init__()
        self.stn = STN3d(channel)
        self.k = 64

        self.conv1_k = torch.nn.Conv1d(channel, self.k, 1)
        self.conv2_k = torch.nn.Conv1d(self.k, 128, 1)
        self.conv3_k = torch.nn.Conv1d(128, 512, 1)

        self.bn1_k = nn.BatchNorm1d(self.k)
        self.bn2_k = nn.BatchNorm1d(128)
        self.bn3_k = nn.BatchNorm1d(512)
...
class PointNetNoTransformEncoder(nn.Module):
    def __init__(self, global_feat=True, feature_transform=False, channel=3):
        super(PointNetNoTransformEncoder, self).__init__()
        self.stn = STN3d(channel)
        self.k = 64
...
def feature_transform_reguliarzer(trans):
    """
    Computes the regularization loss for a feature transformation matrix.

    Parameters:
        trans (torch.Tensor): The feature transformation matrix.

    Returns:
        torch.Tensor: Regularization loss.
    """
    # Get the dimensionality of the transformation matrix
    d = trans.size()[1]

    # Identity matrix with the same dimensions
    i = torch.eye(d)[None, :, :]

    # Move the identity matrix to GPU if the transformation matrix is on GPU
    if trans.is_cuda:
        i = i.cuda()

    # Compute the regularization loss using Frobenius norm
    loss = torch.mean(
